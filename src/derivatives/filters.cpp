...
Matrix forEachPix(const Matrix &img, double(*filterFunc)(uint32_t, uint32_t, const Matrix&, const Matrix&), const Matrix& filterKernel)
{
    ...
    for ( uint32_t row = halfKernelSize; row < img_withZerosInBoards.rows() - halfKernelSize; row++ )
    {
        ...
        {
            filteredImg(row - halfKernelSize, col - halfKernelSize) = filterFunc(row, col, img_withZerosInBoards, filterKernel);
        }
    } 
...
double applyKernelToPix(uint32_t row, uint32_t col, const Matrix& img, const Matrix& filterKernel)
{
    Matrix k = img.block(row - (filterKernel.rows() >> 1), col - ( filterKernel.cols() >> 1 ), filterKernel.rows(), filterKernel.cols());
